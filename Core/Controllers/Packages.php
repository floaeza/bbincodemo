 ...
 */
    ...
    switch ($Option){
        ...
        case 'UpdateGuide':
            ...
            // $resultado = shell_exec('cd /var/www/html/BBINCO/TVCHL/Core/Controllers && python3 DebugTr.py');   
            // $Result= "$resultado\n"; 
            /* Añade redirección, por lo que podemos obtener stderr. */
            // $gestor = popen('cd /var/www/html/BBINCO/TVCHL/Core/Controllers && python3 DebugTr.py', 'r');
            // $leer = fread($gestor, 2096);
            // $Result = $leer;
            // pclose($gestor);
            // $command = escapeshellcmd('/var/www/html/BBINCO/TVCHL/Core/Controllers/DebugTr.py');
            // $output = shell_exec($command);
            $command = escapeshellcmd('/usr/bin/python3 DebugTr.py');
            $output = shell_exec($command);
            $Result = $output;
            // pclose(popen("cd /var/www/html/BBINCO/TVCHL/Core/Controllers && python3 DebugTr.py'","r"));
            // $your_command = 'cd /var/www/html/BBINCO/TVCHL/Core/Controllers && python3 DebugTr.py';
            // shell_exec( $your_command . "> /dev/null 2>/dev/null &" );
            // $IpAddress = '10.0.3.253';
            // $Commandfiles = '/usr/bin/python3 InfomirGrabador.py '.$IpAddress;
            // echo $Commandfiles;
        ...
        case 'InitialConfigurationInfomir':
            $Commandfiles = '/usr/bin/python3 /var/www/html/BBINCO/TVCHL/Core/Controllers/InfomirGrabador.py '.$IpAddress.',register';
            echo $Commandfiles;
            $command = escapeshellcmd($Commandfiles);
            $output = shell_exec($command);
            break;
