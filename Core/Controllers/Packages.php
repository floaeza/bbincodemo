 ...
 */
    ...
    switch ($Option){
        ...
        case 'UpdateGuide':
            ...
            // $resultado = shell_exec('cd /var/www/html/BBINCO/TV3/Core/Controllers && python3 DebugTr.py');   
            // $Result= "$resultado\n"; 
            /* Añade redirección, por lo que podemos obtener stderr. */
            // $gestor = popen('cd /var/www/html/BBINCO/TV3/Core/Controllers && python3 DebugTr.py', 'r');
            // $leer = fread($gestor, 2096);
            // $Result = $leer;
            // pclose($gestor);
            // $command = escapeshellcmd('/var/www/html/BBINCO/TV3/Core/Controllers/DebugTr.py');
            // $output = shell_exec($command);
            $command = escapeshellcmd('sudo /usr/bin/python3 DebugTr.py');
            $output = shell_exec($command);
            // $Result = $output;
            // pclose(popen("cd /var/www/html/BBINCO/TV3/Core/Controllers && python3 DebugTr.py'","r"));
            // $your_command = 'cd /var/www/html/BBINCO/TV3/Core/Controllers && python3 DebugTr.py';
