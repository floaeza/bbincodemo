...
pak = requests.post('http://10.30.0.15/BBINCO/TV_PRUEBAS/Core/Controllers/PY.php', data=payload)
...
def start(day, pos):
    ...
    Version = requests.post('http://10.30.0.15/BBINCO/TV_PRUEBAS/Core/Controllers/PY.php', data=payload)
    Ver = json.loads(Version.content)
    Ver = Ver[0]

    payload = {'Option': 'GetIdentifier'}
    Identifier = requests.post('http://10.30.0.15/BBINCO/TV_PRUEBAS/Core/Controllers/PY.php', data=payload)
    IDF = json.loads(Identifier.content)
    IDF = IDF[0]
    
    payload = {'Option': 'GetOffsetZone'}
    Zone = requests.post('http://10.30.0.15/BBINCO/TV_PRUEBAS/Core/Controllers/PY.php', data=payload)
    OffSetZone = json.loads(Zone.content)
    OffSetZone = OffSetZone[0]

    payload = {'Option': 'GetGatoTime'}
    GTime = requests.post('http://10.30.0.15/BBINCO/TV_PRUEBAS/Core/Controllers/PY.php', data=payload)
    ...
    for ids in range(int(pakk['Pack']), paquetes):
        ...
        x = requests.post('http://10.30.0.15/BBINCO/TV_PRUEBAS/Core/Controllers/PY.php', data=payload)
        ...
        with open('/var/www/html/BBINCO/TV_PRUEBAS/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(ids) + '.json', 'w', encoding='ascii') as file:
            json.dump(data, file, indent=4)

        with open('/var/www/html/BBINCO/TV_PRUEBAS/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(ids) + '.json', 'r') as file:
            filedata = file.read()

        filedata = filedata.replace('[', '').replace(']', '')
        with open('/var/www/html/BBINCO/TV_PRUEBAS/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(ids) + '.json', 'w') as file:
            file.write(filedata)
            print('/var/www/html/BBINCO/TV_PRUEBAS/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(ids) + '.json ', 'CREADO')
...
for pos in range(len(listDays)):
    ...
    req = requests.post('http://10.30.0.15/BBINCO/TV_PRUEBAS/Core/Controllers/Packages.php', data=info)

# os.system ("python3 /var/www/html/BBINCO/TV_PRUEBAS/Core/Controllers/main.py")
