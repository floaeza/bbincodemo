...
Identifier = requests.post('http://'+iplocal+'/BBINCO/TVCHL/Core/Controllers/PY.php', data=payload)
...
files = ls1('/var/www/html/BBINCO/TVCHL/Core/Controllers/Epg/' + IDF['IDF']+'/')
...
for n in range(10):
    ...
    for archivo in files:
        if ('epg_'+DaysToDelete.strftime('%Y%m%d')+'_') in archivo:
            os.remove('/var/www/html/BBINCO/TVCHL/Core/Controllers/Epg/'+IDF['IDF']+'/'+archivo)
            print(archivo)
        else:
            break
...
Pack = requests.post('http://'+iplocal+'/BBINCO/TVCHL/Core/Controllers/Packages.php', data=payload)
...
def start(day, pos):
    ...
    Version = requests.post('http://'+iplocal+'/BBINCO/TVCHL/Core/Controllers/PY.php', data=payload)
    Ver = json.loads(Version.content)
    Ver = Ver[0]
    
    payload = {'Option': 'GetOffsetZone'}
    Zone = requests.post('http://'+iplocal+'/BBINCO/TVCHL/Core/Controllers/PY.php', data=payload)
    OffSetZone = json.loads(Zone.content)
    OffSetZone = OffSetZone[0]

    payload = {'Option': 'GetGatoTime'}
    GTime = requests.post('http://'+iplocal+'/BBINCO/TVCHL/Core/Controllers/PY.php', data=payload)
    ...
    for Package in Packages:
        ...
        x = requests.post('http://'+iplocal+'/BBINCO/TVCHL/Core/Controllers/PY.php', data=payload)
        ...
        with open('/var/www/html/BBINCO/TVCHL/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(Package["id_paquete"]) + '.json', 'w', encoding='ascii') as file:
            json.dump(data, file, indent=4)

        with open('/var/www/html/BBINCO/TVCHL/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(Package["id_paquete"]) + '.json', 'r') as file:
            filedata = file.read()

        filedata = filedata.replace('[', '').replace(']', '')
        with open('/var/www/html/BBINCO/TVCHL/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(Package["id_paquete"]) + '.json', 'w') as file:
            file.write(filedata)
            print('/var/www/html/BBINCO/TVCHL/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(Package["id_paquete"]) + '.json ', 'CREADO')
